...
def evaluateFitness(model: mj.MjModel,
    ...
    if start_x is not None:
        data.qpos[0] = start_x  # robot root x position
        data.qvel[:] = 0.0
    ...
    # --- adaptive speed weighting based on location ---
    # Reduce the importance of speed once the robot reaches the hill
    ...
    np.where(x_mid < x_rugged_end, w_rugged,
             w_uphill)  # everything beyond rugged_end is uphill/steep
)
...
# reward increases as it reaches target_z
    ...
    # vx = np.diff(xs) / dt
    # on_hill = xs[:-1] > x_rugged_end
    # hill_speed_bonus = w_speed_eff * 0.5 * np.sum(np.clip(vx[on_hill], 0, None))  
...
def train_controller(out_csv: Path, # type: ignore
                     ...
                     iterations_phase1: int = 50,
                     iterations_phase2: int = 100,
                     popsize_phase1: int = 128,
                     popsize_phase2: int = 32,
                     hidden: int = 32):
    world, model = build_world_and_model()
    nn_obj = NNController(nq=model.nq, nv=model.nv, nu=model.nu, hidden=hidden, freq_hz=3.0)
    ...
    while not es.stop():
        ...
        gen_time = time.time() - start_gen_time  # end timing
        best_gen_f = -float(np.min(fs))
        history.append(best_gen_f)
        print(f"Phase 1, Generation {es.countiter}, Best fitness: {best_gen_f:.3f}, Time: {gen_time:.2f}s")

    best_x_phase1 = es.result.xbest
    ...
    while not es.stop():
        ...
        best_gen_f = -float(np.min(fs))
        history.append(best_gen_f)
        print(f"Phase 2, Generation {es.countiter}, Best fitness: {best_gen_f:.3f}, Time: {gen_time:.2f}s")
