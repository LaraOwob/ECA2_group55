...
#  BASELINE (Random) 
def run_random_baseline(out_csv, generations=100, pop_size=32, T=10.0, seed=0, model=None, data=None, to_track=None):
    """Evaluate pop_size random rollouts per generation; log best/mean/median to CSV.
    generations : Number of "generations" to run. Each generation = one batch of random rollouts.
    pop_size : Number of random rollouts per generation.
    T : Duration (in seconds) of each rollout.
...
# MLP Controller (shared) for Experiments
class MLPController:
    """
    A 2 layer MLP with tanh activations.
    Total input dimension = nu + 2 (for sin/cos) + 2 (for delta xy) = nu + 4.
    Output: A length-nu control vector, passed through tanh and scaled into [-np.pi / 2, np.pi / 2].
    Frequency: freq_hz controls how fast the sin/cos oscillates.
    """
    def __init__(self, nu: int, hidden: int = 32, freq_hz: float = 1.0):
        """
        nu : model.nu 
        hidden : width of the single hidden layer.
        freq_hz : frequency of the time/phase signal

        in_dim = nu + 4
        hidden -> tanh activation
        out_dim = nu -> tanh activation -> scale to control range
        """
        self.nu = nu
        self.hidden = hidden
        self.in_dim = nu + 4
    ...
    def set_params(self, theta: np.ndarray):
        ...
        # Saves copy of solution, and saves in object
    ...
    def _unpack(self):
        ...
        #W2: (hidden, out_dim)
        #b2: (out_dim,)
        W1 = t[i:i+self.in_dim*self.hidden].reshape(self.in_dim, self.hidden) 
        i += self.in_dim*self.hidden
        b1 = t[i:i+self.hidden] 
        i += self.hidden
        W2 = t[i:i+self.hidden*self.out_dim].reshape(self.hidden, self.out_dim)
        i += self.hidden*self.out_dim
    ...
    def forward(self, prev_ctrl: np.ndarray, t: float, target_pos: np.ndarray, current_pos: np.ndarray) -> np.ndarray:
        ...
        # The 2 layer MLP with tanh activations
        h = np.tanh(obs @ W1 + b1)
        y = np.tanh(h @ W2 + b2)
        return CTRL_RANGE * y  # Scale to control range
...
def main(experiment,
    ...
    """Main function to run the simulation with random movements.
      ...
      - "plot":Generate 3 plots overlaying Random baseline"""
    ...
    if experiment == "baseline":
        ...
        # Visualize one baseline rollout in the viewer
