...
SPAWN_RUGGED = [ 1.4, 0.0, 0.1]  # mid arena
SPAWN_UPHILL = [ 3.6, 0.0, 0.2]  # near start of uphill
...
# ===== CPG Brain (Phase A) ============================================= #
# --- CPG params: [a(ν), phi(ν), b(ν), omega(ν)]  <-- per-joint ω
def _sample_cpg_params(nu: int, rng: np.random.Generator = RNG) -> npt.NDArray[np.float64]:
    amp_max = np.pi/3            # a bit gentler than 0.9*(π/2); avoids self-collisions
    a   = rng.uniform(0.15*amp_max, amp_max, size=nu)

    # symmetry-bias phases: alternate ~0 and ~π with small noise
    base = np.where(np.arange(nu) % 2 == 0, 0.0, np.pi)
    phi  = (base + rng.uniform(-0.25, 0.25, size=nu)) % (2*np.pi)

    b   = rng.uniform(-0.35, 0.35, size=nu)      # tighter offsets than ±π/4
    omega = rng.uniform(1.2, 3.5, size=nu)       # per-joint frequency
...
class CPGBrain:
    ...
    def forward(self, model: mj.MjModel, data: mj.MjData) -> npt.NDArray[np.float64]:
        ...
        omega = self.params[3*nu:4*nu]           # vector ω
        t = float(data.time)
...
def training_fitness_from_history(
    ...
    arr = np.asarray(xpos_history, dtype=np.float64)
    xs, ys, zs = arr[:,0], arr[:,1], arr[:,2]
    T = max(len(xs), 1)

    # progress & speed
    progress = float(np.max(xs))                               # monotonic incentive
    forward_vel = float(np.sum(np.maximum(0.0, np.diff(xs)))) / max(T-1, 1)

    # early-finish bonus (scaled by how early you reach)
    _base_fit, reached, t_hit = fitness_from_history_time_or_distance(xpos_history, dt, target_x)
    bonus = 0.0
    if reached and t_hit is not None:
        episode_T = T * dt
        bonus = 2.0 + 8.0 * max(0.0, 1.0 - t_hit / (episode_T + 1e-6))  # in [2,10]

    # regularizers
    center_pen   = 0.5 * float(np.mean(np.abs(ys)))                     # stay centered
    off_lane_pen = 3.0 if float(np.max(np.abs(ys)) > lane_half_width) else 0.0
    fall_pen     = 5.0 if float(np.min(zs) < z_floor) else 0.0
    jerk_pen     = 0.01 * float(np.mean(np.abs(np.diff(np.diff(xs)))))  # discourage jerks

    # maximize this
    return (1.0*progress + 0.3*forward_vel + bonus) - (center_pen + off_lane_pen + fall_pen + jerk_pen)
...
def curriculum_spawns(best_x: float) -> list[list[float]]:
    if best_x < -0.4:   return [SPAWN_FLAT]
    if best_x <  1.0:   return [SPAWN_FLAT, SPAWN_RUGGED]
    return [SPAWN_FLAT, SPAWN_RUGGED, SPAWN_UPHILL]

def duration_schedule(best_x: float) -> int:
    if best_x < -0.4: return 20
    if best_x <  1.0: return 60     # give more time once you’re moving
    if best_x <  2.5: return 90
    return 140
...
def _cpg_bounds(nu: int):
    ...
    return {
        ...
        "b":     (np.full(nu, -0.35),    np.full(nu,  0.35)),
        "omega": (np.full(nu, 1.0),      np.full(nu,  4.5)),
    }

def _clip_cpg_params(theta: npt.NDArray[np.float64], nu: int) -> npt.NDArray[np.float64]:
    a_lo,a_hi = _cpg_bounds(nu)["a"]
    b_lo,b_hi = _cpg_bounds(nu)["b"];   w_lo,w_hi = _cpg_bounds(nu)["omega"]
    out = theta.copy()
    out[0:nu]        = np.clip(out[0:nu],        a_lo, a_hi)
    out[nu:2*nu]     = np.mod(out[nu:2*nu],      2*np.pi)
...
def cma_train_cpg_on_body(
    ...
    lo = np.concatenate([np.zeros(nu), np.zeros(nu), np.full(nu, -0.35), np.full(nu, 1.0)])
    hi = np.concatenate([np.full(nu, np.pi/3), np.full(nu, 2*np.pi), np.full(nu, 0.35), np.full(nu, 4.5)])
    ...
    opts = {
        ...
        "bounds": [lo, hi],
        "popsize": popsize,
        "CMA_stds": (hi - lo) * 0.25,   # slightly bolder search
        "CMA_active": True,
        "CMA_mirrors": 1,
        "verb_disp": 0,
    }
    ...
    for g in range(1, iters + 1):
        ...
        active_spawns = curriculum_spawns(best_x_seen)
        ...
        for x in X:
            ...
            f, hist, x_end = evaluate_cpg_multi_spawn(session, theta, duration=dur, spawns=active_spawns)
...
def cma_train_with_restarts(core, n_restarts=3, iters=20, sigma0=0.3, pop0=16):
    best = None
    pop = pop0
    for r in range(n_restarts):
        theta, fit, hist, session = cma_train_cpg_on_body(core, iters=iters, sigma0=sigma0, popsize=pop)
        if best is None or fit > best[1]:
            best = (theta, fit, hist, session)
        pop = int(pop * 1.7)         # increase diversity
        sigma0 *= 0.9                # slightly tighter search on restart
    return best
...
def main() -> None:
    ...
    core,
    iters=60,       # try 30–80; more if budget allows
    sigma0=0.30,    # like your ES sigma
    seed=SEED,
    popsize=24     # try 12–32; CMA default also fine
)
    # ? ------------------------------------------------------------------ #

    nu = session.model.nu
    assert best_params.shape[0] == 4 * nu, f"Expected 4*nu params, got {best_params.shape}"

    # --- FINAL EVAL from the real start on a longer horizon ---
    final_fit, reached, t_hit, history = rollout_cpg_on_session(
        session,
        best_params,
        duration=150,         # longer horizon for the final check
        spawn_pos=SPAWN_FLAT  # explicit spawn at the true start
    )

    np.save(DATA / "gecko_cpg_best.npy", best_params)
    np.save(DATA / "gecko_cpg_final_hist.npy", np.asarray(history))
