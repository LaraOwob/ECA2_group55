...
CHECKPOINT2_X =  1.0    # roughly beyond rugged region (tune) before it was 2.0
...
def es_train_cpg_on_body(
    ...
    best_fit, best_hist, best_x = evaluate_cpg_multi_spawn(session, parent, duration=30)
...
def main() -> None:
    ...
    best_params, best_fit, best_hist, session, run_dir = es_train_cpg_on_body(
        core,
        iters=80,      # before it was 28, you can increase later
        lam=24,        # before it was 12
        sigma=0.35,    # a bit bolder; your 1/5th rule will adapt it (before it was 0.25 )
        seed=SEED,
    ...
    # final eval from true start with long duration
    # session = make_session(core)
    final_fit, reached, t_hit, history = rollout_cpg_on_session(session, best_params, duration=100)
